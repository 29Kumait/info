          ...
          >
            {images.map((image) => (
              <DraggableImage
                key={image.fileId}
                image={image}
                position={imagePositions[image.fileId]}
                rotationAngle={getRotationAngle(image.fileId)}
...
const StaticImage = React.memo(function StaticImage({
  ...
  return (
    <div style={style} className="ring-offset-white img-frame transition-transform ease-in-out duration-300 shadow-xl">
      <div className="absolute top-1 right-1 text-sm">🧷</div>
      <img src={image.url} alt={image.name} className="w-full h-full object-contain rounded-md" />
    </div>
  );
