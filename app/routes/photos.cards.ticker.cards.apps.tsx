...
export default function AppData() {
    ...
    return (
        ...
        >
            ...
            <div className="max-w-5xl mx-auto p-8 rounded-xl shadow-lg justify-evenly bg-gray-900/60 m-5">
                { apps.length > 0 ? (
                    apps.map ((app) => (
                        <div key={ app.id }  className="mt-12 ">
                            <h2 className="prose m-6 text-2xl p-6 transition-transform hover:drop-shadow-blue-glow">
                                <a className="prose font-bold p-4 w-48 h-12 flex items-center justify-center border-2 border-[#FF507A] rounded-lg text-[#BEE0DB] hover:bg-[#FF507A] hover:text-white no-underline transition-all" href={ app.deployment }>
                                    { app.title }
                                </a>
                            </h2>
